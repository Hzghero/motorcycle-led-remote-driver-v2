...
static uint8_t  g_tx_boot_wkup_pending_key = TLIGHT_KEY_NONE; /* STANDBY 唤醒补发单击：L/M/R */
...
static void TxKeys_Process(uint32_t now, uint8_t tick_1ms)
{
  ...
  /* 低功耗唤醒补偿（三键）：
   * 仅在对应按键已释放时补发单击；若仍按下则判定为长按场景并跳过注入。 */
